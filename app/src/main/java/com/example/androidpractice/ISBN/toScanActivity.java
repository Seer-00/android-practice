...
public class toScanActivity extends AppCompatActivity {

    private static String TAG = toScanActivity.class.getName();
    ...
    private void startScanner() {
        // 原始扫描
        /*
        IntentIntegrator integrator = new IntentIntegrator(toScanActivity.this);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if ((result == null) || (result.getContents() == null)) {
            Log.i(toScanActivity.TAG, "Cancel scanning");
        ...
        Log.i(toScanActivity.TAG, "Scanning result: " + contents);
    ...
    private class DownloadThread extends Thread {
        ...
        public DownloadThread(String url) {
            super();
            this.url = url;
            Log.i(toScanActivity.TAG, "Success: create download thread");
        }

        @Override
        public void run() {
            Log.i(toScanActivity.TAG, "Success: run download thread");
    ...
    private static class DownloadHandler extends Handler {
        ...
        private toScanActivity toScanActivity;

        public DownloadHandler(toScanActivity toScanActivity) {
            super();
            this.toScanActivity = toScanActivity;
        }

        @Override
        public void handleMessage(@NonNull Message msg) {
            if ((msg.obj == null) || (toScanActivity.mProgressDialog == null)
                    || (!toScanActivity.mProgressDialog.isShowing())) {
                return;
            }

            toScanActivity.mProgressDialog.dismiss();

            Response response = (Response) msg.obj;

            if (response.getResCode() != BookAPI.RESPONSE_CODE_SUCCEED) {
                // 通信异常处理
                Log.i(toScanActivity.TAG, "Failure: Communicating ["
                        + response.getResCode() + "]: " + response.getResMessage());

                Toast.makeText(toScanActivity, "状态码:" + response.getResCode() + " "
                        + response.getResMessage(), Toast.LENGTH_LONG).show();
            } else {
                // 通信正常，转到图书信息界面
                Log.i(toScanActivity.TAG, "Success: Communicating");
                toScanActivity.startBookInfoDetailActivity((BookInfo) response.getResMessage());
            }
        }
    }
